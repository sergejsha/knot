...
class CompositionTest {
    ...
    fun `CompositeKnot picks reducers by change type`() {
        val composition = Composition<State, Change, Action>().apply {
            reducers[Change.A::class] = { change, state -> Effect(state.copy(value = change.value)) }
            reducers[Change.B::class] = { change, state -> Effect(state.copy(value = change.value)) }
        }
    ...
    fun `CompositeKnot performs actions`() {
        ...
        val composition = Composition<State, Change, Action>().apply {
            ...
            reducers[Change.A::class] = { change, state -> Effect(state.copy(value = change.value), Action.A) }
            reducers[Change.ADone::class] = { change, state -> Effect(state.copy(value = change.value)) }
            reducers[Change.B::class] = { change, state -> Effect(state.copy(value = change.value), Action.B) }
            reducers[Change.BDone::class] = { change, state -> Effect(state.copy(value = change.value)) }
        }
