...
class KnotInterceptStateTest {
    ...
    @Test
    fun `intercept { state } receives initial State`() {
        ...
        knot<State, Change, Action> {
            ...
            intercept {
                state { state -> state.doOnNext { interceptor.onNext(it) } }
            }
        }
    ...
    @Test
    fun `intercept { state } receives State mutations`() {
        val interceptor = PublishSubject.create<State>()
        val observer = interceptor.test()
    ...
    @Test
    fun `intercept { state } receives State mutations in multiple statements`() {
        val interceptor = PublishSubject.create<State>()
        val observer = interceptor.test()
        val knot = knot<State, Change, Action> {
            state {
